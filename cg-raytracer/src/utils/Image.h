...
public:
    ...
    explicit Image(unsigned int _width = 0, unsigned int _height = 0);
    ...
    /// image(x,y) = color;
    vec3& operator()(const unsigned int _x, const unsigned int _y)
    {
        assert(_x < width_);
        assert(_y < height_);
        const unsigned int el = _y * static_cast<unsigned int>(width_) + _x;
        return pixels_[el];
    }

    /// Read access to pixel (_x,_y).
    const vec3& operator()(const unsigned int _x, const unsigned int _y) const
    {
        assert(_x < width_);
        assert(_y < height_);
...
private:
    ...
    unsigned short int width_ = 0;

    /// image height in pixels
    unsigned short int height_ = 0;
};
