 ...
 * You can specify command line arguments by using the terminal inside VSCode
...
* **Textures:**
  One color per object is boring. Complete the texture support in `Mesh::intersect_triangle()` and render our cool Pokémon scene (`scenes\pokemon\pokemon.sce`).

* **Acceleration:**
Are you annoyed because ray-tracing is damn slow? Let's make things faster! Implement the bounding box test for triangle meshes (`Mesh::intersect_bounding_box()` in `Mesh.cpp`). With this feature you should see a significant performance boost rendering the toon faces (`scenes\toon_faces\toon_faces.sce`).
